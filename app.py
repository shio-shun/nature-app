...
origin = st.selectbox("🌏 起点にしたい観光スポットを選んでください", df["スポット名"].tolist())
...
folium.Marker(
    ...
    popup=f"{origin}<br><a href='{origin_row['公式サイト']}' target='_blank'>公式サイト</a><br>所要時間: {origin_row['所要時間']}",
    tooltip=f"起点：{origin}",
...
for _, row in recommended.iterrows():
    folium.Marker(
        ...
        popup=f"{row['スポット名']}<br><a href='{row['公式サイト']}' target='_blank'>公式サイト</a><br>所要時間: {row['所要時間']}",
        tooltip=row["スポット名"]
    ).add_to(m)
...
st.subheader("📍 近隣のおすすめスポット")
st.table(recommended[["スポット名", "距離_km", "所要時間"]].reset_index(drop=True))
